...
files = {'Sim':('jobcomp.log','JobIDRaw','|'),
        'Live':('sacct_output.csv','JobIDRaw','|'),
        'Trace':('test_trace.csv','sim_job_id',','),
        'Canc':('cancelled_jobs.csv','JobIDRaw',',')}
...
# JOIN ALL
datetime_columns ={ 'End','Start','Submit','sim_submit','Eligible','CancelTime'}
for name,df in dfs:
...
print(f"STATEMENT: Job durations (live and sim) differ more than {str(threshold)}")
print(f"STATEMENT: only for cancelled jobs:")
print(f"STATEMENT: STATUS : {result}")


# CORE-SECONDS CHECK
core_seconds_live = (joined.durationLive/np.timedelta64(1,'s')*joined.NCPUSLive).sum() 
core_seconds_sim  = (joined.durationSim/np.timedelta64(1,'s')*joined.NCPUSSim).sum() 
print(f"INFO     : Total core seconds on the live system: {core_seconds_live}")
print(f"INFO     : Total core seconds on the simulated system: {core_seconds_sim}")
core_seconds_sim2  = (joined.durationSim/np.timedelta64(1,'s')*joined.NCPUSLive).sum() 
print(f"INFO     : Total core seconds on the simulated system (*): {core_seconds_sim2}")


# CANCEL TIME CHECKS
condition = (abs(joined.CancelTime - joined.EndLive)<threshold) | pd.isna(joined.CancelTime)
result=all(condition)
print(f"STATEMENT: For the jobs cancelled in the sim, ends differ only by {str(threshold)} max")
print(f"STATEMENT: between the cancellation request to the simulator and the live data:")
print(f"STATEMENT: STATUS : {result}")


threshold = np.timedelta64(5,'m')
job_simulation_ends_before_live = ((joined.EndSim - joined.EndLive)<threshold)
condition = job_simulation_ends_before_live | (~job_is_cancelled_live)
result=all(condition)
print(f"STATEMENT: For the jobs cancelled in the sim, they end before their live")
print(f"STATEMENT: counterpart or max after {str(threshold)} the live counterpart:")
print(f"STATEMENT: STATUS : {result}")
...
result=all(condition)
print(f"STATEMENT: Jobs that were cancelled in the live system last less than ")
...
core_seconds_live_cancelled = (joined.durationLive/np.timedelta64(1,'s')*joined.NCPUSLive)[job_is_cancelled_live].sum() 
core_seconds_sim_cancelled  = (joined.durationSim/np.timedelta64(1,'s')*joined.NCPUSSim)[job_is_cancelled_live].sum() 
print(f"INFO     : Total core seconds on the live system in cancelled jobs: {core_seconds_live_cancelled}")
print(f"INFO     : Total core seconds on the simulated system in cancelled jobs: {core_seconds_sim_cancelled}")
core_seconds_sim_cancelled2  = (joined.durationSim/np.timedelta64(1,'s')*joined.NCPUSLive)[job_is_cancelled_live].sum() 
print(f"INFO     : Total core seconds on the simulated system in cancelled jobs (*): {core_seconds_sim_cancelled2}")
